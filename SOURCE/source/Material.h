...
#pragma region Material COOK TORRENCE
	...
	public:
		...
		ColorRGB Shade(const HitRecord& hitRecord = {}, const Vector3& l = {}, const Vector3& v = {}) override
		{
			...
			ColorRGB specular{  (fresnel * geometry * normalDistribution) * 
								(1.0f / (4.0f * Vector3::Dot(v, hitRecord.normal) *
								Vector3::Dot(l, hitRecord.normal))) };
			...
			ColorRGB lambert{};
			if (m_Metalness < FLT_EPSILON) { lambert = BRDF::Lambert({ 1.0f - fresnel.r, 1.0f - fresnel.g,  1.0f - fresnel.b }, m_Albedo); }
			else { lambert = BRDF::Lambert(0, m_Albedo); }
